...
const runtimeEdge = new RuntimeEdge({ initialCode })
const response = await runtimeEdge.dispatchFetch('https://example.vercel.sh')
...
const server = await runServer({ runtime: runtimeEdge, port: 3000 })
